...
func testUserToManyDynamics(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, dynamicDBTypes, false, dynamicColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, dynamicDBTypes, false, dynamicColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Dynamics().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadDynamics(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Dynamics); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Dynamics = nil
	if err = a.L.LoadDynamics(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Dynamics); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyFolders(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.Folders().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadFolders(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Folders); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Folders = nil
	if err = a.L.LoadFolders(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Folders); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyImageOfCovers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, imageOfCoverDBTypes, false, imageOfCoverColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, imageOfCoverDBTypes, false, imageOfCoverColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ImageOfCovers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadImageOfCovers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ImageOfCovers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ImageOfCovers = nil
	if err = a.L.LoadImageOfCovers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ImageOfCovers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyImages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadImages(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyImpressions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadImpressions(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyMarks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadMarks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpDynamics(t *testing.T) {
	var err error
	...
	foreigners := []*Dynamic{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, dynamicDBTypes, false, strmangle.SetComplement(dynamicPrimaryKeyColumns, dynamicColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Dynamic{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDynamics(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Dynamics[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Dynamics[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Dynamics().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpFolders(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.R.Folders[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Folders[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Folders().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpImageOfCovers(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e ImageOfCover

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ImageOfCover{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, imageOfCoverDBTypes, false, strmangle.SetComplement(imageOfCoverPrimaryKeyColumns, imageOfCoverColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ImageOfCover{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddImageOfCovers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ImageOfCovers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ImageOfCovers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ImageOfCovers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpImages(t *testing.T) {
	var err error
...
func testUserToManyAddOpImpressions(t *testing.T) {
	var err error
...
func testUserToManyAddOpMarks(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`UserID`: `text`, `Name`: `character varying`, `Email`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)

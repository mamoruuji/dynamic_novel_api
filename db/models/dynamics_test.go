...
func testDynamicToManyDynamicsOnTerms(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := DynamicSlice{&a}
	if err = a.L.LoadDynamicsOnTerms(ctx, tx, false, (*[]*Dynamic)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testDynamicToManyAddOpDynamicsOnTerms(t *testing.T) {
	var err error

...
func testSectionToOneTermUsingTerm(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.TermID, foreign.TermID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}
	...
	if !queries.Equal(check.TermID, foreign.TermID) {
	...
	slice := SectionSlice{&local}
	if err = local.L.LoadTerm(ctx, tx, false, (*[]*Section)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Term == nil {
		t.Error("struct should have been eager loaded")
...
func testSectionToOneSetOpTermUsingTerm(t *testing.T) {
	var err error
	...
	for i, x := range []*Term{&b, &c} {
		...
		if !queries.Equal(a.TermID, x.TermID) {
...
func testSectionToOneRemoveOpTermUsingTerm(t *testing.T) {
	var err error
	...
	if err = a.SetTerm(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveTerm(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.Term().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if !queries.IsValuerNil(a.TermID) {
		t.Error("foreign key value should be nil")
	}

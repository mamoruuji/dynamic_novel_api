...
var PositionWhere = struct {
	...
	Name:       whereHelperstring{field: "\"positions\".\"name\""},
	CreatedAt:  whereHelpertime_Time{field: "\"positions\".\"created_at\""},
	UpdatedAt:  whereHelpertime_Time{field: "\"positions\".\"updated_at\""},
}
...
func (r *positionR) GetSections() SectionSlice {
	if r == nil {
		return nil
	}
	return r.Sections
...
var (
	positionAllColumns            = []string{"position_id", "name", "created_at", "updated_at"}
	positionColumnsWithoutDefault = []string{"name", "updated_at"}
	positionColumnsWithDefault    = []string{"position_id", "created_at"}
...
func (o *Position) Sections(mods ...qm.QueryMod) sectionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"sections\".\"position_id\"=?", o.PositionID),
	)
...
func (positionL) LoadSections(ctx context.Context, e boil.ContextExecutor, singular bool, maybePosition interface{}, mods queries.Applicator) error {
	var slice []*Position
	var object *Position

	if singular {
		var ok bool
		object, ok = maybePosition.(*Position)
		if !ok {
			object = new(Position)
			ok = queries.SetFromEmbeddedStruct(&object, &maybePosition)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybePosition))
			}
		}
	} else {
		s, ok := maybePosition.(*[]*Position)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybePosition)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybePosition))
			}
		}
	}
	...
	query := NewQuery(
		qm.From(`sections`),
		qm.WhereIn(`sections.position_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Position) AddSections(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Section) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.PositionID, rel.SectionID}
...
// Sets o.R.Position's Sections accordingly.
// Replaces o.R.Sections with related.
// Sets related.R.Position's Sections accordingly.
func (o *Position) SetSections(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Section) error {
	query := "update \"sections\" set \"position_id\" = null where \"position_id\" = $1"
...
// Sets related.R.Position.
func (o *Position) RemoveSections(ctx context.Context, exec boil.ContextExecutor, related ...*Section) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("position_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
...
func (o *Position) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return PositionExists(ctx, exec, o.PositionID)
}

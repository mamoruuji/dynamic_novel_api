...
func (o *Dynamic) DynamicsOnTags(mods ...qm.QueryMod) dynamicsOnTagQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"dynamics_on_tags\".\"dynamic_id\"=?", o.DynamicID),
	)

	return DynamicsOnTags(queryMods...)
}
...
// LoadDynamicsOnTags allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (dynamicL) LoadDynamicsOnTags(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDynamic interface{}, mods queries.Applicator) error {
	var slice []*Dynamic
	var object *Dynamic
	...
	query := NewQuery(
		qm.From(`dynamics_on_tags`),
		qm.WhereIn(`dynamics_on_tags.dynamic_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load dynamics_on_tags")
	}

	var resultSlice []*DynamicsOnTag
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice dynamics_on_tags")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on dynamics_on_tags")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for dynamics_on_tags")
	}

	if len(dynamicsOnTagAfterSelectHooks) != 0 {
		for _, obj := range resultSlice {
			if err := obj.doAfterSelectHooks(ctx, e); err != nil {
				return err
			}
	...
	if singular {
		object.R.DynamicsOnTags = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &dynamicsOnTagR{}
			}
			foreign.R.Dynamic = object
		}
		return nil
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.DynamicID == foreign.DynamicID {
				local.R.DynamicsOnTags = append(local.R.DynamicsOnTags, foreign)
				if foreign.R == nil {
					foreign.R = &dynamicsOnTagR{}
				}
				foreign.R.Dynamic = local
...
// AddDynamicsOnTags adds the given related objects to the existing relationships
// of the dynamic, optionally inserting them as new records.
// Appends related to o.R.DynamicsOnTags.
// Sets related.R.Dynamic appropriately.
func (o *Dynamic) AddDynamicsOnTags(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DynamicsOnTag) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.DynamicID = o.DynamicID
			...
			updateQuery := fmt.Sprintf(
				"UPDATE \"dynamics_on_tags\" SET %s WHERE %s",
				strmangle.SetParamNames("\"", "\"", 1, []string{"dynamic_id"}),
				strmangle.WhereClause("\"", "\"", 2, dynamicsOnTagPrimaryKeyColumns),
			)
			values := []interface{}{o.DynamicID, rel.DynamicsOnTagsID}
	...
	if o.R == nil {
		o.R = &dynamicR{
			DynamicsOnTags: related,
		}
	} else {
		o.R.DynamicsOnTags = append(o.R.DynamicsOnTags, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &dynamicsOnTagR{
				Dynamic: o,
			}
		} else {
			rel.R.Dynamic = o

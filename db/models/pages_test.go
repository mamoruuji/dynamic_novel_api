...
func testPagesExists(t *testing.T) {
	...
	e, err := PageExists(ctx, tx, o.PageID)
	if err != nil {
...
func testPagesFind(t *testing.T) {
	...
	pageFound, err := FindPage(ctx, tx, o.PageID)
	if err != nil {
		t.Error(err)
	}
...
func testPageToManySections(t *testing.T) {
	...
	slice := PageSlice{&a}
	if err = a.L.LoadSections(ctx, tx, false, (*[]*Page)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPageToOneChapterUsingChapter(t *testing.T) {
	...
	local.ChapterID = foreign.ChapterID
	...
	if err = local.L.LoadChapter(ctx, tx, false, (*[]*Page)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPageToOneSetOpChapterUsingChapter(t *testing.T) {
	var err error
	...
	for i, x := range []*Chapter{&b, &c} {
		...
		if a.ChapterID != x.ChapterID {
			t.Error("foreign key was wrong value", a.ChapterID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ChapterID))
		reflect.Indirect(reflect.ValueOf(&a.ChapterID)).Set(zero)
		...
		if a.ChapterID != x.ChapterID {
			t.Error("foreign key was wrong value", a.ChapterID, x.ChapterID)
		}
	}
}
...
var (
	pageDBTypes = map[string]string{`PageID`: `integer`, `Title`: `character varying`, `Order`: `integer`, `ChapterID`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)

...
var (
	prismaMigrationDBTypes = map[string]string{`ID`: `character varying`, `Checksum`: `character varying`, `FinishedAt`: `timestamp with time zone`, `MigrationName`: `character varying`, `Logs`: `text`, `RolledBackAt`: `timestamp with time zone`, `StartedAt`: `timestamp with time zone`, `AppliedStepsCount`: `integer`}
	_                      = bytes.MinRead
)
